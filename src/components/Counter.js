...
export default function Counter() {
  /* STEP 1 
  Using the state hook, create a count, setCount, pair. The 'count' state should be initialized to the number zero. */
  
  const [count, setCount] = useState(0)

  const increment = () => {
    /* STEP 4 
    This click handler need to use  setCount to schedule the 'count to become the current count + 1. THe se state changes are not synchronous: the updated count arrives on the next run of the Counter component. Do NOT simply do count ++. The plus plus is forbidden! We never mutate a slice of state in place. Even if you could reassign a const, React wold not be aware anything changed. Always use the state updater, passing in a new value.*/
    setCount(count + 1)
  };
  const decrement = () => {
    /* STEP 5 
    The click handler needs to use set Count to se the 'count' to be the current count -1. Do NOT do count --! That amounts to trying to mutate 'count' in place.*/
    setCount(count - 1)
  };
  const reset = () => {
    /* STEP 6 
    This click handler needs to use setCount to set the count to be 0 again.*/
    setCount(0)
  };
  ...
  const style = {
    ...
    The 'style' object has the 'color' property hard-coded to "royalblue". What the value of 'color' should be instead is a ternary (?) expression that goes like this: If count is even, then "royalblue", else, "crimson" */
  };
  ...
  return (
    <div className='widget-counter container'>
      ...
      <div id='count' style={style}>
        ...
        {/* STEP 3
        Replace some hard-coded info in the JSX with expressions, interpolated inside curly brackets. Start by replacing the character "0" with {count}. The 'count' slice of state is the source of truth here. */}
      </div>
      <div>
